...
-- =============================================
-- PROVINCES (reference table, no RLS needed)
-- =============================================
...
CREATE TABLE exercises (
  ...
  exercise_type TEXT NOT NULL CHECK (exercise_type IN (
    'multiple_choice','drag_drop','match_pairs','fill_blank','listening','pronunciation'
  )),
  order_index INTEGER NOT NULL,
...
-- Profiles: users can read all public fields, write only their own
CREATE POLICY "profiles_select" ON profiles FOR SELECT USING (true);
CREATE POLICY "profiles_update_own" ON profiles FOR UPDATE USING (auth.uid() = id);

-- Lessons/Modules: all authenticated users can read published content
...
-- User-specific tables: own rows only
